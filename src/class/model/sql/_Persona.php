...
class _PersonaSql extends EntitySql{
  ...
  public function _mappingField($field){
    ...
    switch ($field) {
      ...
      case $p.'actualizado_ym': return "DATE_FORMAT({$t}.actualizado, '%Y-%m')";
      case $p.'actualizado_y': return "DATE_FORMAT({$t}.actualizado, '%Y')";
      case $p.'informado': return $t.".informado";
      case $p.'informado_date': return "CAST({$t}.informado AS DATE)";
      case $p.'informado_ym': return "DATE_FORMAT({$t}.informado, '%Y-%m')";
      case $p.'informado_y': return "DATE_FORMAT({$t}.informado, '%Y')";
      ...
      case $p.'avg_informado': return "AVG({$t}.informado)";
      case $p.'min_informado': return "MIN({$t}.informado)";
      case $p.'max_informado': return "MAX({$t}.informado)";
      case $p.'count_informado': return "COUNT({$t}.informado)";
...
' . $this->_mappingField($p.'id') . ', ' . $this->_mappingField($p.'region') . ', ' . $this->_mappingField($p.'distrito') . ', ' . $this->_mappingField($p.'localidad') . ', ' . $this->_mappingField($p.'programa') . ', ' . $this->_mappingField($p.'inscripcion_men') . ', ' . $this->_mappingField($p.'cens') . ', ' . $this->_mappingField($p.'sede') . ', ' . $this->_mappingField($p.'comision') . ', ' . $this->_mappingField($p.'tramo') . ', ' . $this->_mappingField($p.'identificador') . ', ' . $this->_mappingField($p.'numero_inscripcion') . ', ' . $this->_mappingField($p.'numero_documento') . ', ' . $this->_mappingField($p.'apellidos') . ', ' . $this->_mappingField($p.'nombres') . ', ' . $this->_mappingField($p.'genero') . ', ' . $this->_mappingField($p.'telefono') . ', ' . $this->_mappingField($p.'correo') . ', ' . $this->_mappingField($p.'trayectoria_educativa') . ', ' . $this->_mappingField($p.'turno') . ', ' . $this->_mappingField($p.'pc_escritorio') . ', ' . $this->_mappingField($p.'net_notebook') . ', ' . $this->_mappingField($p.'tablet') . ', ' . $this->_mappingField($p.'pc_tablet') . ', ' . $this->_mappingField($p.'impresora') . ', ' . $this->_mappingField($p.'conexion_internet_paga') . ', ' . $this->_mappingField($p.'telefono_celular') . ', ' . $this->_mappingField($p.'comision_2020') . ', ' . $this->_mappingField($p.'archivo_2019') . ', ' . $this->_mappingField($p.'archivo_2020') . ', ' . $this->_mappingField($p.'drive') . ', ' . $this->_mappingField($p.'cuil') . ', ' . $this->_mappingField($p.'analitico_dni') . ', ' . $this->_mappingField($p.'analitico_cuil') . ', ' . $this->_mappingField($p.'analitico_partida') . ', ' . $this->_mappingField($p.'analitico_certificado') . ', ' . $this->_mappingField($p.'fecha_nacimiento') . ', ' . $this->_mappingField($p.'ingreso') . ', ' . $this->_mappingField($p.'observaciones') . ', ' . $this->_mappingField($p.'activo') . ', ' . $this->_mappingField($p.'fila') . ', ' . $this->_mappingField($p.'error') . ', ' . $this->_mappingField($p.'id_comision') . ', ' . $this->_mappingField($p.'actualizado') . ', ' . $this->_mappingField($p.'informado') . '';
  ...
  public function _conditionFieldStruct($field, $option, $value){
    ...
    case "{$p}activo_is_set": return $this->format->conditionIsSet($this->_mappingField("{$p}activo"), $value, $option);
      ...
      case "{$p}actualizado": return $this->format->conditionDateTime($this->_mappingField($field), $value, $option, "Y-m-d H:i:s");
      case "{$p}actualizado_date": return $this->format->conditionDateTime($this->_mappingField($field), $value, $option, "Y-m-d");
      case "{$p}actualizado_is_set": return $this->format->conditionIsSet($this->_mappingField("{$p}actualizado"), $value, $option);
      case "{$p}actualizado_ym": return $this->format->conditionDateTimeAux($this->_mappingField($field), $value, $option, "Y-m");
      case "{$p}actualizado_y": return $this->format->conditionDateTimeAux($this->_mappingField($field), $value, $option, "Y");

      case "{$p}informado": return $this->format->conditionDateTime($this->_mappingField($field), $value, $option, "Y-m-d H:i:s");
      case "{$p}informado_date": return $this->format->conditionDateTime($this->_mappingField($field), $value, $option, "Y-m-d");
      case "{$p}informado_is_set": return $this->format->conditionIsSet($this->_mappingField("{$p}informado"), $value, $option);
      case "{$p}informado_ym": return $this->format->conditionDateTimeAux($this->_mappingField($field), $value, $option, "Y-m");
      case "{$p}informado_y": return $this->format->conditionDateTimeAux($this->_mappingField($field), $value, $option, "Y");
      ...
      case "{$p}avg_informado": return $this->format->conditionNumber($this->_mappingField($field), $value, $option);
      case "{$p}avg_informado_is_set": return $this->format->conditionIsSet($this->_mappingField("{$p}avg_informado"), $value, $option);

      case "{$p}max_informado": return $this->format->conditionNumber($this->_mappingField($field), $value, $option);
      case "{$p}max_informado_is_set": return $this->format->conditionIsSet($this->_mappingField("{$p}max_informado"), $value, $option);

      case "{$p}min_informado": return $this->format->conditionNumber($this->_mappingField($field), $value, $option);
      case "{$p}min_informado_is_set": return $this->format->conditionIsSet($this->_mappingField("{$p}min_informado"), $value, $option);

      case "{$p}count_informado": return $this->format->conditionNumber($this->_mappingField($field), $value, $option);
      case "{$p}count_informado_is_set": return $this->format->conditionIsSet($this->_mappingField("{$p}count_informado"), $value, $option);
  ...
  public function format(array $row){
    ...
    if(array_key_exists('actualizado', $row)) $row_['actualizado'] = $this->format->dateTime($row['actualizado'], "Y-m-d H:i:s");
    if(array_key_exists('informado', $row)) $row_['informado'] = $this->format->dateTime($row['informado'], "Y-m-d H:i:s");
